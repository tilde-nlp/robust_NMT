...
def parse_args():
    ...
    parser.add_argument('-i', '--input', help='Input(test) file', required=True, nargs='+')
...
def main():
    ...
    for input_file in args.input:
        ...
        ilines = [util.preprocess(x, args.lang) for x in codecs.open(input_file, 'r', 'utf-8').readlines()]
        ...
        for lineno, (rline, iline) in enumerate(itertools.izip(ilines, rlines), start=1):
            ...
            if len(scores) <= lineno - 1:
                scores.append([])
            scores[lineno - 1].append(score)
            if args.verbose:
                print("Sentence {0}: {1:.4f}".format(lineno, score))
    scores = map(lambda x: sum(x) / len(x), scores)
